...
import { getEnvironments } from "../_lib/queries"
...
export default async function DataTableEnvironment({ 
  searchParams = {} 
}: DataTableEnvironmentProps) {
  const { data, count, error } = await getEnvironments(searchParams)
  ...
  const tableDeleteAction = deleteEnvironments as (ids: string[]) => Promise<{ success: boolean; error?: string; deletedCount?: number }>
  const tableCreateAction = createEnvironment as unknown as (data: Record<string, unknown>) => Promise<{ success: boolean; error?: string }>
  const tableUpdateActionSingle = updateEnvironment as unknown as (id: string, data: Record<string, unknown>) => Promise<{ success: boolean; error?: string }>
  const tableUpdateActionMulti = multiUpdateEnvironments as unknown as (ids: string[], data: Record<string, unknown>) => Promise<{ success: boolean; error?: string; updatedCount?: number }>
  
  // Cast the custom forms to match expected types
  const AddForm = EnvironmentAddForm as React.ComponentType<{
    onSuccess?: () => void
    onCancel?: () => void
    createAction?: (data: Record<string, unknown>) => Promise<{ success: boolean; error?: string }>
  }>
  
  const EditFormSingle = EnvironmentEditForm as React.ComponentType<{
  ...
  const EditFormMulti = EnvironmentMultiEditForm as React.ComponentType<{
